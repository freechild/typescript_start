    ...
    if (projectResult != false) {
      chapter.bulkWrite(req.body, function (chapterResult) {
        if(chapterResult == true)
          ...
          res.send("SUBJECT DB ERROR")
      });
    }else
      res.send("PROJECT DB ERROR")
  });
};
  ...
  project.readOneDocument(project_id,body.recipient,function(result){
    try {
      ...
      if(result.length > 2)
        ...
        var sendData = {}
        sendData.name = result[0].Board.name,
        sendData.title = result[0].Board.title,
        sendData.content = result[0].Board.content,
        sendData.recipient = body.recipient;
