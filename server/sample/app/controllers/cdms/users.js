...
module.exports.editUser = function (req, res, next) {
  ...
  if (body.projectNum) {
    ...
    //recursive function
    var callbackFnc = function callbackFnc(arr, returnVal) {
      var shifted = arr.shift();
      project.findOneByNameCount(shifted, function (result) {
        if (result == 0) {
          if (returnVal == "") returnVal += shifted;else returnVal += "," + shifted;
        }
        if (arr.length) {
          ...
          if (returnVal == "") {
            ...
            try {
              for (var _iterator = removeItem[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
