...
projectSchema.statics.findAll = function (cb) {
  ...
  var condition = { "_id": -1 };
  this.aggregate([{ $sort: { '_id': -1 } }, { $limit: 20 }]).exec(function (err, members) {
    if (err) throw err;
    if (members) {
      cb(members);
    } else {
...
projectSchema.statics.addSubjects = function (body, cb) {
  ...
  for (var i = 1; i <= count; ++i) {
    ...
    if (pattern != "") pattern += "|^" + data._id;else pattern = "^" + data._id;
  }

  this.update({ _id: body.project }, {
    $set: { subjects: [] }
  }, { "multi": true }, function (err, result) {
    that.update({ _id: body.project }, {
      $push: {
        subjects: {
          $each: pushData,
          $sort: { index: 1 }
    ...
    $project: { Board: 1, arrayIndexindex: 1, BoardSize: 1,
      ...
      "Date": {
        "$let": {
          "vars": {
            "local_time": {
              "$subtract": ["$Board.regdate", -32400000]
            }
          },
          "in": {
            "year": {
